...
public:
  ...
  geometry_msgs::PoseStamped in_initObjectPose;
  geometry_msgs::PoseStamped in_goalObjectPose;
  //output data
...
private:
  ...
  ros::Time lastUpdateInitObjectPose_;
  ros::Time lastUpdateGoalObjectPose_;
...
public:
  hpp_impl() : problemSolver_ (new ProblemSolver()),
    ...
    lastUpdateInitObjectPose_.sec = 0;
    lastUpdateInitObjectPose_.nsec = 0;
    lastUpdateGoalObjectPose_.sec = 0;
    lastUpdateGoalObjectPose_.nsec = 0;
  ...
  void update(hpp_data &data, hpp_config config)
  {
    ...
    if (robot) {
      if (!initConfig_ || initConfig_->size () != robot->configSize ()) {
	initConfig_ = ConfigurationPtr_t (new Configuration_t
					  (robot->configSize ()));
	*initConfig_ = robot->neutralConfiguration ();
      }
      if (!goalConfig_ || goalConfig_->size () != robot->configSize ()) {
	goalConfig_ = ConfigurationPtr_t (new Configuration_t
					  (robot->configSize ()));
	*goalConfig_ = robot->neutralConfiguration ();
      }
      ...
      assert (robot->rootJoint () &&
	      robot->rootJoint ()->numberChildJoints () >= 2);
      // Make asumption that object freeflyer joint is second child of root
      // joint.
      size_type objectRankInConfig =
	robot->rootJoint ()->childJoint (1)->rankInConfiguration ();
      if (data.in_initObjectPose.header.stamp != lastUpdateInitObjectPose_) {
        hpp::ros::objectPoseToConfig
	  (objectRankInConfig, data.in_initObjectPose.pose, *initConfig_);
      }
      if (data.in_goalObjectPose.header.stamp != lastUpdateGoalObjectPose_) {
        hpp::ros::objectPoseToConfig
	  (objectRankInConfig, data.in_goalObjectPose.pose, *goalConfig_);
      }
    }
